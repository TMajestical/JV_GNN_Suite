 ...
 * @brief Graph Neural Networks Suite for Vertex classification
 ...
 Note: The training function for GNN is StarPlat generated from GNN DSL and corresponding main must be used. These files would be present in StarPlat/graphcode/staticDSLCodes and StarPlat/graphcode/main_functions.


 gnn_obj.saveModelTxt(filename);  to save the model into a text file for viewing.
 gnn_obj.saveModel(filename); to save the model into a binary file, suitable for loading.

 while creating a model by loading gnn_obj GNN_algo(filename, dataset_obj); needs to be used.
 */
